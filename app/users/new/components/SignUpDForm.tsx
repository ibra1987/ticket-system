...
import { FormErrorType, NewCustomerAccountFields,NewProfessionalAccountFields } from "@/types"
import { AccountType } from "@/types"
import { FormEvent, useState } from "react"

function SignUpDForm() {
    const  [accountType,setAccountType]=useState<AccountType>(AccountType.customer)
    const [validationErrors,setValidationErrors]= useState<FormErrorType[] | undefined>([])
    ...
    const onSubmit=async(e:FormEvent)=>{
      ...
      e.preventDefault();
      const newUser  = accountType === AccountType.customer ? newCustomer as NewCustomerAccountFields : newProfessional as NewProfessionalAccountFields
    ...
    const getErrorMessages = (path:string)=>{
      ...
      const errorMessages = validationErrors?.find((error:FormErrorType)=>error.path === path)?.errorMessages
  ...
  return (
    <form onSubmit={onSubmit} className="w-full flex flex-col justify-start items-center md:w-1/2   py-10">
      ...
      <Input
        ...
        {accountType === AccountType.customer && Object.keys(newCustomer).map((field:string,index:number)=>(
         <div className="w-full"  key={`${field}-${index}`}>
          <Input 
          inputType={newCustomer[field as keyof NewCustomerAccountFields].type}

         
          class={getErrorMessages(field) && getErrorMessages(field)!.length ? " p-2 w-full   outline-none focus:border-2 focus:border-blue-300 rounded border-2 border-red-500" :" p-2 w-full border-2  outline-none focus:border-2 focus:border-blue-300 rounded "}
          ...
          {validationErrors && validationErrors?.length>0 &&<ErrorContainer errorMessages={getErrorMessages(field)!}/> }
          </div>
        ))}
        {
          accountType === AccountType.Professional && (<>
                ...
                {Object.keys(newProfessional).map((field:string,index)=>(
                  ...
                  className="w-full">
                     <Input 
                   inputType={newProfessional[field as keyof NewProfessionalAccountFields].type}
                   name={field}
                   class={getErrorMessages(field) && getErrorMessages(field)!.length ? "m-2 p-2 w-full   outline-none focus:border-2 focus:border-blue-300 rounded border-2 border-red-500" :"m-2 p-2 w-full border-2  outline-none focus:border-2 focus:border-blue-300 rounded "}
